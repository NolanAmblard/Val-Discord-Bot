...
public class Tempban implements Commands {
    ...
    public void execute(List<String> args, MessageReceivedEvent event) {
        ...
        if (message.length == 3) {
            ...
            event.getGuild().getMemberByTag(username).ban(duration);
            event.getChannel().sendMessage("User " + username + " has been banned for " + duration + " days.");
        }
        ...
        if (message.length == 4) {
            ...
            event.getGuild().getMemberByTag(username).ban(duration, reason);
            event.getChannel().sendMessage("User " + username + " has been banned for " + duration + " days. Reason: " + reason);
        }
    }
